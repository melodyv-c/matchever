document.addEventListener("DOMContentLoaded", () => {
  ...
  /* ==========================================
     3. BOUTON RIVE DU HERO
     ========================================== */
  const riveCatButton = document.getElementById("rive-cat-button");
  const riveCatCanvas = document.getElementById("rive-cat-canvas");
  const prefersReducedMotion = window.matchMedia(
    "(prefers-reduced-motion: reduce)",
  ).matches;

  if (riveCatButton) {
    riveCatButton.addEventListener("click", () => {
  ...
  if (
    ...
    let riveCat = null;
    const riveStateMachine = riveCatButton.dataset.riveStateMachine;

    riveCat = new window.rive.Rive({
      src: riveCatButton.dataset.riveSrc || "button.riv",
      canvas: riveCatCanvas,
      ...
      onLoad: () => {
        riveCat.resizeDrawingSurfaceToCanvas(2);
        riveCatButton.classList.add("is-ready");
  ...
  if (cardDeck) {
    ...
    const cards = Array.from(cardDeck.querySelectorAll(".swipe-card"));
    ...
    function updateActiveCard() {
      ...
      const visibleCards = cardDeck.querySelectorAll(
        ".swipe-card:not(.swiped)",
      );
  ...
  /* ==========================================
     5. WIDGET DE MINI-QUIZ DE PERSONNALITE
     ========================================== */
  ...
  if (quizWidget) {
    ...
    function setupStepOptions(stepNum, answerKey, nextBtn) {
      const stepEl = quizWidget.querySelector(
    ...
    function goToStep(currentStep, targetStep) {
      const currentEl = quizWidget.querySelector(
        `.quiz-step[data-step="${currentStep}"]`,
      );
      const targetEl = quizWidget.querySelector(
        `.quiz-step[data-step="${targetStep}"]`,
      );
    ...
    if (submitBtn) {
      submitBtn.addEventListener("click", () => {
        // Cacher l'étape 3
        quizWidget
          .querySelector('.quiz-step[data-step="3"]')
    ...
    const petProfiles = {
      ...
      dogCalm: {
        ...
        reason:
          "✓ Idéal pour les appartements avec balcon et présence modérée.",
      ...
      catActive: {
        ...
        reason:
          "✓ Recommandé pour les appartements et les personnes très présentes.",
      ...
      catCalm: {
        ...
        reason:
          "✓ Excellent pour les appartements calmes et les rythmes professionnels classiques.",
    ...
    function showQuizResult() {
      ...
      if (answers.activity === "sportif" && answers.space === "jardin") {
        ...
        answers.activity === "sportif" ||
        answers.space === "jardin" ||
        answers.space === "balcon"
      ) {
      ...
      document.getElementById("result-pet-img").alt =
        `${match.name}, ${match.meta}`;
      ...
      document.getElementById("result-compatibility-reason").textContent =
        match.reason;
  ...
  /* ==========================================
     6. SIMULATEUR D'ADOPTION INTERACTIF (Scrollytelling)
     ========================================== */
